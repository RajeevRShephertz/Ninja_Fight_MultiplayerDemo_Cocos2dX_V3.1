...
void HelloWorld::onConnectDone(int res)
{
    if (res==AppWarp::ResultCode::success)
    {
        ...
        prop["Cat"]="catch";
        warpClientRef->joinRoomInUserRange(1, 3, true);
        //warpClientRef->createTurnRoom("Gamers", "Rajeev", 4, prop, 60);
    }
...
void HelloWorld::onUserJoinedRoom(AppWarp::room rData, std::string user)
{
    if (rData.result == 0)
    {
        printf("\nonUserJoinedRoom...SUCCESS");
        printf("\nuser=%s",user.c_str());
    }
    else
    {
        printf("\nonUserJoinedRoom...FAILED");
    }
    printf("\nResult=%d",rData.result);
    printf("\nroomID=%s",rData.roomId.c_str());
    printf("\nUser=%s",user.c_str());
    printf("\nName=%s",rData.name.c_str());
}
...
void HelloWorld::onJoinRoomDone(AppWarp::room revent)
{
    ...
    else
    {
        ...
        std::map<std::string,std::string> prop;
        prop["Cat"]="catch";
        AppWarp::Client::getInstance()->createTurnRoom("Gamer", "Ranjan", 4, prop, 60);
    }
}
